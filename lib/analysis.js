...
const IGNORED_OBSERVATION_PATTERNS = [
  ...
  /^\s*\[cron:[^\]]+\]/iu,
  /^\s*write a dream diary entry from these memory fragments:/iu,
];
...
export function isIgnoredObservationText(text) {
  return IGNORED_OBSERVATION_PATTERNS.some((pattern) => pattern.test(text));
}

export function isHeartbeatText(text) {
  return isIgnoredObservationText(text);
}
...
export function buildObservation({ messages, maxExcerptChars, runId, agentId, sessionKey, timestamp }) {
  ...
  if (isIgnoredObservationText(rawText)) return null;
